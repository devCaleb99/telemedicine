...
const PersonnelLogin = () => {
  ...
  const signUp = async () => {
    ...
    try {
      ...
      // Save to Firestore personnel collection
      ...
      console.log('Personnel data saved to Firestore');
      setFormData({ firstName: '', lastName: '', email: '', password: '', authorizationCode: '', specialization: '' });
      setErrors({});
      setSubmitted(false);
      navigate('/chat');
    } catch (error) {
  ...
  const signInUser = async () => {
    ...
    try {
      ...
      console.log('User signed in:', { uid: userCredential.user.uid, email: userCredential.user.email });
      setFormData({ firstName: '', lastName: '', email: '', password: '', authorizationCode: '', specialization: '' });
      setErrors({});
      setSubmitted(false);
      navigate('/chat');
  ...
  const signInWithGoogle = async () => {
    ...
    setSubmitted(true);
    const googleErrors = {
      authorizationCode: validate.authorizationCode(formData.authorizationCode),
      specialization: validate.specialization(formData.specialization),
    };
    setErrors(googleErrors);
    if (googleErrors.authorizationCode || googleErrors.specialization) {
      setIsLoading(false);
      return;
    }
    try {
  ...
  const validate = {
    firstName: (value) => {
      ...
      if (value.length > 30) return 'Name too long';
      return '';
    },
    lastName: (value) => {
    ...
    authorizationCode: (value) => {
      ...
      if (!/^icare$/.test(value)) return 'Not authorized';
      return '';
    },
    specialization: (value) => {
      if (!value.trim()) return ''; // Optional
      if (value.length < 3) return 'Specialization too short';
      return '';
    },
  };
  ...
  const validateForm = () => {
    ...
    if (activeForm === 'signup') {
      ...
      newErrors.authorizationCode = validate.authorizationCode(formData.authorizationCode);
  ...
  return (
    <section className="nunito container mx-auto md:py-20 md:px-15 bg-gradient-to-r from-purple-600 to-pink-500">
      <motion.div
        ...
        className="bg-white/20 backdrop-blur-lg h-[100vh] shadow-glass rounded-lg p-5 max-w-md mx-auto"
      >
        {/* Toggle Switch */}
            ...
            >
              ...
              >
                <input
                  ...
                  onChange={handleChange}
                  className={`placeholder-black p-3 text-xs font-bold rounded-lg bg-white/20 backdrop-blur-lg shadow-glass focus:ring-2 focus:ring-white/50 focus:outline-none transition-all w-full ${
                    shouldShowError('specialization') ? 'border-red-500 border' : ''
                  }`}
              ...
              >
                <input
                  ...
                  onChange={handleChange}
                  className={`placeholder-black p-3 text-xs font-bold rounded-lg bg-white/20 backdrop-blur-lg shadow-glass focus:ring-2 focus:ring-white/50 focus:outline-none transition-all w-full ${
                    shouldShowError('authorizationCode') ? 'border-red-500 border' : ''
                  }`}
                  placeholder="Authorization code (optional)"
                />
                {shouldShowError('authorizationCode') && (
                  <motion.p
                    initial={{ opacity: 0, y: -5 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="text-red-500 text-xs mt-1"
